...
def master(layers, neurons_in_each_layer, epochs,input_dim, output_dim, x, y):
    ...
    #adding layers
    add_layer(number_of_neurons=16,context='sigmoid',input_dim=input_dim)
    add_layer(number_of_neurons=8,context='sigmoid')
    add_layer(number_of_neurons=output_dim,context='softmax')

    global gradient
    """Copy the structure of of network to gradient nd transient gradient."""
    gradient = copy.deepcopy(network)
    global transient_gradient
    transient_gradient = copy.deepcopy(network)
    train(datapoints=trainX, labels=trainy, epochs=epochs, f=input_dim)


master(layers=3, neurons_in_each_layer=8, epochs=50, input_dim=784,output_dim=10, x=trainX, y=trainy)
